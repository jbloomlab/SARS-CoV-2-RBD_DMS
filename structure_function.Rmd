...
p1 <- ggplot(mutants, aes(x=RBM,y=bind_avg))+
...
p2 <- ggplot(mutants, aes(x=RBM,y=expr_avg))+
  geom_violin(trim=F)+coord_flip()+
  stat_summary(fun.y=median,geom="point",size=1)+
...
To create a surface representation of the RBD colored by mutational tolerance (in this case, white to red, to mimic the heatmaps we produce below), execute the following commands in a PyMol session with one of these output `pdb` files loaded:
```
create RBD, chain E
