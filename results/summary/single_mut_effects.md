...
(Still work in progress, so you won’t see all of this completed here,
yet\!)
...
Next, for the global episitasis and tobit regression models, let’s look
at the correlation between model coefficients inferred from lib1 and
lib2 binding measurements. We look at the model coefficients both on the
“observed” log<sub>10</sub>(*K*<sub>A,app</sub>) and underlying “latent”
scales, as well as two examples of models without any nonlinearity
correction.
...
multi-mutants in the library. For the global epistasis models, the
Cauchy likelihood models show better correlation between replicates.
This may be in part because the “shape” of global epistasis differed
between libraries in the two Gaussian likelihood fits. The “shape” looks
good in the Tobit regression fits, but the model coefficients themselves
appear to exhibit more variability from the regression line.
...
First, let’s look at the correlation between these drieclty measured
...
#tobit, observed
x <- rowMeans(betas_bind_observed_tobit[,c("effect_lib1","effect_lib2")]);y <- NULL; for(i in 1:nrow(betas_bind_observed_tobit)){y <- c(y, betas[mutation_RBD==betas_bind_observed_tobit$mutation[i],mean(c(bind_lib1_direct,bind_lib2_direct))])};fit <- lm(y~x)
...
#tobit, latent
x <- rowMeans(betas_bind_latent_tobit[,c("effect_lib1","effect_lib2")]);y <- NULL; for(i in 1:nrow(betas_bind_latent_tobit)){y <- c(y, betas[mutation_RBD==betas_bind_latent_tobit$mutation[i],mean(c(bind_lib1_direct,bind_lib2_direct))])};fit <- lm(y~x)
plot(x,y,pch=16,col="#00000067",xlab="average lib1&lib2 beta",ylab="direct single mut measurement",main=paste("Directly measured vs Tobit, latent scale.\n R-squared:",round(summary(fit)$r.squared,digits=3)))
```
...
in the lower three plots). This lends further support to the idea that
the *observed* scale effects are our model coefficients of interest, and
