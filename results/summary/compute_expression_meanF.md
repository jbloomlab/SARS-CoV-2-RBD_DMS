...
for fewer than 15 cells, and return the number and fraction of barcodes
...
counts_filtered_lib1[total_count < 15, c("ML_meanF","var_ML_meanF") := as.numeric(NA),by=barcode]
counts_filtered_lib2[total_count < 15, c("ML_meanF","var_ML_meanF") := as.numeric(NA),by=barcode]
...
```
    ...
    ## [1] "Generated meanF estimates for 94.73% (94395) of lib1 barcodes"
...
```
    ...
    ## [1] "Generated meanF estimates for 92.8% (90653) of lib2 barcodes"
...
SARS-CoV-2 targets only. We also report a delta-expression metric,
delta\_ML\_meanF, by subtracting the median expression of WT and
...
counts_filtered_lib1$delta_ML_meanF <- counts_filtered_lib1$ML_meanF - median(counts_filtered_lib1[variant_class %in% c("wildtype","synonymous"),ML_meanF],na.rm=T)
counts_filtered_lib2$delta_ML_meanF <- counts_filtered_lib2$ML_meanF - median(counts_filtered_lib2[variant_class %in% c("wildtype","synonymous"),ML_meanF],na.rm=T)
