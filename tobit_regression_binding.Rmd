...
packages = c("yaml","data.table","tidyverse","censReg","AER","VGAM","caret")
...
dt_1_tofit <- dt_1[-which(matrix_1[,matrix_1_linearCombos$remove]==1),]
#and remove these columns from the model matrix
matrix_1_tofit <- matrix_1[,-matrix_1_linearCombos$linearCombos[[1]]]
...
#first tried using VGAM package tobit function, on counts_1. This probably got the closest to a fit I've got (for lib1, at least -- the censReg implementation seems to be running on lib2)
#it failed saying it can only run on full-rank models -- suggesting at least one of the mutations in my library is non-identifiable
#would probably be a low count, and perhaps only observed on a background with another set of mutations?
tobit_1_vglm <- vglm(dt_1_tofit$log10Ka ~ matrix_1_tofit, tobit(Lower=6,imethod=3), trace=T)
save(tobit_1_vglm, file=paste(config$tobit_regression_binding_dir,"/tobit_1_vglm.Rda",sep=""))

tobit_2_vglm <- vglm(dt_2_tofit$log10Ka ~ matrix_2_tofit, tobit(Lower=6,imethod=3), trace=T)
save(tobit_2_vglm, file=paste(config$tobit_regression_binding_dir,"/tobit_2_vglm.Rda",sep=""))

# tobit_1 <- censReg(dt_1$log10Ka ~ matrix_1, left=6)
# save(tobit_1,file=paste(config$tobit_regression_binding_dir,"/tobit_1.Rda",sep=""))
# tobit_2 <- censReg(dt_2$log10Ka ~ matrix_2, left=6) #this is currently being fit and saved as tobit_2 by the script running through snakemake
# save(tobit_2,file=paste(config$tobit_regression_binding_dir,"/tobit_2.Rda",sep=""))

#try out tobit() function in the AER package
# tobit_1_AER <- tobit(dt_1$log10Ka ~ matrix_1, left = 6)
