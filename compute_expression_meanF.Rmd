...
Finally, let's output our measurements for downstream analyses. Since only the SARS-CoV-2 data is going into the global epistasis models, we will output separate files, for all barcodes, and for barcodes for SARS-CoV-2 targets only. We also report a delta-expression metric, delta_ML_meanF, by subtracting the median expression of WT and synonymous variants from each ML_meanF metric. This should calibrate measurements between the two libraries for fitting joint global epistasis models, in which the average wildtype expression is different by a very small amount.
